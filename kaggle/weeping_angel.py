...
specification = {
    ...
    "observation": {
        "angels": {
            "description": "Red-only. [id, file, status] per Angel (0=lurk,1=caught,2=corrupt).",
            "type": "array",
            "default": [],
        },
        "coverageLog": {
...
def interpreter(state, env):
    ...
    if env.done:
        ...
        red.observation.angels = [[i, f, LURKING] for i, f in enumerate(files)]
    ...
    acted_files = set()
    for a in angels:
        aid, f, status = a[0], a[1], a[2]
        if status == LURKING and aid in moves:
            acted_files.add(f)
            if f in cover:
                a[2] = CAUGHT
    ...
    # still-lurking Angel is noisier than an empty one.
    lurking_files = {a[1] for a in angels if a[2] == LURKING}
    act_rng = random.Random(config.seed * 100003 + tick)  # nosec B311
    for f in range(config.nFiles):
        if f in acted_files:
            continue
        rate = config.angelRate if f in lurking_files else config.benignRate
        if act_rng.random() < rate and f in cover:
            log.append([tick, f, 0])
